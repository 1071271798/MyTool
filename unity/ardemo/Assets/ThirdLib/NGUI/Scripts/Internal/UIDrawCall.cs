...
public class UIDrawCall : MonoBehaviour
{
	...
	void CreateMaterial ()
	{
        ...
        catch (System.Exception ex)
        {
            ...
            MyLog.LogError(this.GetType() + "-" + st.GetFrame(0).ToString() + "- error = " + ex.ToString());
        }
	...
	Material RebuildMaterial ()
	{
		...
		if (mTexture != null && mDynamicMat != null) mDynamicMat.mainTexture = mTexture;

		// Update the renderer
		if (mRenderer != null && mDynamicMat != null) mRenderer.sharedMaterials = new Material[] { mDynamicMat };
		return mDynamicMat;
	}
