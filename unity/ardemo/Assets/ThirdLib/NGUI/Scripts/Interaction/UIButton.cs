...
public class UIButton : UIButtonColor
{
    ...
    /// 播放的音效
    /// </summary>
    public SoundEffect soundEffect = SoundEffect.Normal_Btn;
	/// <summary>
    ...
    bool mOnGrey = false;
    UILabel mLabel;
    //Color mSpriteOldColor = Color.white;
	/// <summary>
	...
	protected override void OnInit ()
	{
        ...
        mLabel = transform.GetComponentInChildren<UILabel>();
        if (mSprite != null) { mNormalSprite = mSprite.spriteName;/* mSpriteOldColor = mSprite.color; */}
		if (mSprite2D != null) mNormalSprite2D = mSprite2D.sprite2D;
        if (mOnGrey)
        {
            //OnGrey();
	...
	protected virtual void OnClick ()
	{
		...
		if (current == null && isEnabled)
		{
            if (playSound)
            {
                SingletonBehaviour<AudioManager>.GetInst().Play(soundEffect);
            }
			current = this;
    ...
    public virtual void OnGrey(Color color)
    {
        if (null != mSprite)
        {
            mSprite.OnGrey(color);
        }
        else
        {
            mOnGrey = true;
    ...
    public virtual void UnGrey(Color color)
    {
        if (null != mSprite)
        {
            mSprite.UnGrey(color);
        }
    }
    ...
    public void OnSleep()
    {
        ...
        {
            UISprite[] sp = transform.GetComponentsInChildren<UISprite>();
            if (null != sp)
            {
                for (int i = 0, imax = sp.Length; i < imax; ++i)
                {
                    sp[i].alpha = 0.5f;
                }
            }
    ...
    public void OnSleep(Color color)
    {
        enabled = false;
        SetColliderActive(false);
        //OnGrey(color);
    ...
    public void OnAwake(Color color)
    {
        enabled = true;
        SetColliderActive(true);
        //UnGrey(color);
