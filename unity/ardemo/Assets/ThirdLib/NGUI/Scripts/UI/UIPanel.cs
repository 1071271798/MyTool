...
public class UIPanel : UIRect
{
	...
	public virtual Vector3 CalculateConstrainOffset (Vector2 min, Vector2 max)
	{
		...
		{
			minArea.x += clipSoftness.x + 1;
			minArea.y += clipSoftness.y + 1;
			maxArea.x -= clipSoftness.x + 1;
			maxArea.y -= clipSoftness.y + 1;
		}
		return NGUIMath.ConstrainRect(minRect, maxRect, minArea, maxArea);
	}

    public virtual Vector3 CalculateEndOffset(Vector2 min, Vector2 max)
    {
        Vector4 cr = finalClipRegion;
    ...
    public virtual Vector3 CalculateFrontOffset(Vector2 min, Vector2 max)
    {
        Vector4 cr = finalClipRegion;
    ...
    public int IsInPanel(Bounds bs)
    {
        Vector4 cr = finalClipRegion;
        ...
        Vector2 minRect = new Vector2(bs.min.x, bs.min.y);
        Vector2 maxRect = new Vector2(bs.max.x, bs.max.y);
        Vector2 minArea = new Vector2(cr.x - offsetX, cr.y - offsetY);
        Vector2 maxArea = new Vector2(cr.x + offsetX, cr.y + offsetY);
        if (minRect.x < minArea.x || minRect.y < minArea.y)
        {
            return -1;
        }
        else if (maxRect.x > maxArea.x || maxRect.y > maxArea.y)
        {
            return 1;
        }
        return 0;
    ...
    public int GetInPanelArea(Bounds bs, bool horizontal, bool vertical, int space = 15)
    {
        Vector4 cr = finalClipRegion;
        ...
        if (horizontal)
        {
            if (minRect.x < minArea.x || minRect.x - minArea.x < space)
            {
                return -1;
            }
            else if (maxRect.x > maxArea.x || maxArea.x - maxRect.x < space)
            {
                return 1;
            }
        }
        if (vertical)
        {
            if (minRect.y < minArea.y || minRect.y - minArea.y < space)
            {
                return -1;
            }
            else if (maxRect.y > maxArea.y || maxArea.y - maxRect.y < space)
            {
                return 1;
            }
        }
