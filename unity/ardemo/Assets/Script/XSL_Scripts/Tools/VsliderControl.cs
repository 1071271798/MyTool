...
public class VsliderControl : MonoBehaviour
{
    ...
    void OnPress(bool isPressed)
    {
        ...
        SliderWidgetData sliderData = null;
        if (ControllerManager.GetInst().widgetManager.vSliderManager.GetVSliderByID(gameObject.name) != null)
            sliderData = (ControllerManager.GetInst().widgetManager.vSliderManager.GetVSliderByID(gameObject.name)).sliderData;
        DuoJiData djData = null;
        if (sliderData != null)
        {
            djData = RobotManager.GetInst().GetCurrentRobot().GetAllDjData().GetDjData((byte)sliderData.servoID);
        }
        if (sliderData != null && sliderData.motionType == CtrlMotionType.servo && null != djData && djData.modelType == ServoModel.Servo_Model_Angle)
        {
            if (isPressed)
            {
                Lighting(1f);
