...
public class Joystick : MonoBehaviour
{
    ...
    void OnPress(bool isPressed)
    {
        ...
        if (joyControl != null && joyControl.joyData.type == JockstickData.JockType.twoServo)
        {
            wheelServoID01 = joyControl.joyData.leftUpID;
            wheelServoID02 = joyControl.joyData.rightUpID;
            wheelServoID03 = 0;
            wheelServoID04 = 0;
        }
        else if (joyControl != null && joyControl.joyData.type == JockstickData.JockType.fourServo)
        {
            wheelServoID01 = joyControl.joyData.leftUpID;
            wheelServoID02 = joyControl.joyData.rightUpID;
            wheelServoID03 = joyControl.joyData.leftBottomID;
            wheelServoID04 = joyControl.joyData.rightBottomID;
        }

        if (null == joyControl || joyControl.joyData.motionType == CtrlMotionType.servo && ((wheelServoID01 != 0 && RobotManager.GetInst().GetCurrentRobot().GetAllDjData().GetDjData((byte)wheelServoID01).modelType == ServoModel.Servo_Model_Angle) ||
        ...
        else
        {
            if (isPressed)
            {
                ...
                if (joyControl.isReady)
                {
                    ...
                    joyControl.TurnWhellSpeed(thumb.transform.localPosition.x, thumb.transform.localPosition.y, radius);
                }
                else
                {
            ...
            else
            {
                if (joyControl != null && joyControl.isReady)
                {
                    ...
                    joyControl.TurnWhellSpeed(0, 0, radius);
                }
                CalculateJoystickAxis();
                if (On_JoystickMoveEnd != null)
                {
    ...
    void SetJoystickSize(int radius)
    {
        ...
        thumb.width = (int)(49.7f / 100f * root.width);
        thumb.height = (int)(49.7f / 100f * root.height);
    }
