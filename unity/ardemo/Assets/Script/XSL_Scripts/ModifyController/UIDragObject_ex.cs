...
public class UIDragObject_ex : MonoBehaviour
{
    EventDelegate.Callback dragStartCallback;
    EventDelegate.Callback dragOverCallback;
    ...
    void OnEnable()
    {
        ...
        if (isClone && target != null)
        {
            Transform p = target.parent;
            Transform t = Instantiate(target) as Transform;
            Destroy(t.GetComponent<UIDragObject_ex>());
    ...
    public void OnPress(bool pressed)
    {
        ...
        if (enabled && NGUITools.GetActive(gameObject) && target != null)
        {
            ...
            if (!pressed)
            {
                Debug.Log("dragoverCallback");
                if (dragOverCallback != null) //拖动结束回调
                    dragOverCallback();
            }
        }
    }
    ...
    protected void OnDragStart()
    {
        ...
        if (dragStartCallback != null)
            dragStartCallback();
        dragOverCallback = TestDragOverEvent;
    }

    void TestDragOverEvent()
    {
        Debug.Log("drag voer");
    }
    ...
    /// <summary>
    /// 触发器，移动时碰到触发时表明此处不可放置，否则可放置，
    /// 1，是否可放置判断：进入触发时变量++，出来时--，lateupdate判断变量是否为0表示是否可放置
    /// 2，被移动的对象碰撞器设置完全，被碰撞的碰撞器长宽缩小1/2格 
    /// </summary>
    public delegate void triggleDelegate();
    public triggleDelegate triggleDel;
