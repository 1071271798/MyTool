...
public class RapidBlurEffect:SingletonBehaviour<RapidBlurEffect>
{
    //-------------------变量声明部分-------------------
    #region Variables
    ...
    //着色器和材质实例
    public static Shader CurShader;
    public Material CurMaterial;
    private bool ScreenshotsFlag;
    ...
    void OnRenderImage(RenderTexture sourceTexture, RenderTexture destTexture)
    {
        ...
        if (CurShader != null)
        {
            ...
            RenderTexture renderBuffer = RenderTexture.GetTemporary(renderWidth, renderHeight, 0);
            //设置渲染模式：双线性
            ...
            for (int i = 0; i < BlurIterations; i++)
            {
                ...
                // 获取临时渲染纹理
                tempBuffer = RenderTexture.GetTemporary(renderWidth, renderHeight, 0);
        ...
        if (!ScreenshotsFlag)
        {
            SingletonBehaviour<Screenshots>.GetInst().SetScreenshots(mTexture, delegate ()
            {
                if (null != mTexture)
                {
                    mTexture.alpha = 0;
    ...
    public static void ShowRapidBlurEffect()
    {
        ...
        if (null != cameraAttb && null != cameraAttb.camera)
        {
            ...
            SetRapidBlurEffect(ShowTexture.GetUITexture(), null);
        }
    }

    public static void SetRapidBlurEffect(UITexture uiTexture, EventDelegate.Callback onFinished)
    {
        UICameraAttb cameraAttb = UIManager.GetInst().GetCameraData(eUICameraType.OrthographicTwo);
        if (null != cameraAttb && null != cameraAttb.camera)
        {
            SingletonBehaviour<Screenshots>.GetInst().SetScreenshots(uiTexture, delegate ()
            {
                ...
                RapidBlurEffect effect = cameraAttb.camera.gameObject.GetComponent<RapidBlurEffect>();
                if (null != effect)
                {
                    GameObject.DestroyImmediate(effect);
                }
                effect = cameraAttb.camera.gameObject.AddComponent<RapidBlurEffect>();
