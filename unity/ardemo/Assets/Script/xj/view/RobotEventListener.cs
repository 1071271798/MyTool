...
/// <summary>
/// Author:xj
/// FileName:RobotEventListener.cs
/// Description:模型点击监听
/// Time:2015/7/10 10:31:58
/// </summary>
public class RobotEventListener : MonoBehaviour
{
    ...
    public readonly static Color Select_Color = Color.green;
    //public readonly static Color Normal_Color = new Color32(82, 82, 82, 255);
    public static Color Normal_Color;
    public static MoveSecond mJms;

    public delegate void OnDragDelegate(GameObject obj, Vector2 delta);
    public delegate void OnClickDelegate(GameObject obj);

    public static OnDragDelegate onDragDelegate;
    public static OnClickDelegate onSelectDelegate;

    /// <summary>
    /// List of all active cameras in the scene.
    /// </summary>

    static public BetterList<RobotEventListener> list = new BetterList<RobotEventListener>();
    ...
    public LayerMask eventReceiverMask = 11;
    ...
    static public RobotEventListener current = null;
    ...
    static GameObject mCurrentSelectDj = null;
    public static GameObject CurrentSelectDj
    {
        get { return mCurrentSelectDj; }
    }

    static bool mCanChangeIDFlag = true;
    static bool mCanChangeRotaFlag = true;
    ...
    static public GameObject selectedObject
    {
        ...
        set
        {
            SetSelection(value, RobotEventListener.currentScheme);
        }
    }
    ...
    static protected void SetSelection(GameObject go, ControlScheme scheme)
    {
        ...
        else if (mCurrentSelection != go)
        {
            ...
            if (RobotEventListener.list.size > 0)
            {
                RobotEventListener cam = (mNextSelection != null) ? FindCameraForLayer(mNextSelection.layer) : RobotEventListener.list[0];
    ...
    System.Collections.IEnumerator ChangeSelection()
    {
        ...
        if (mCurrentSelection != null)
        {
            current = this;
            currentCamera = mCam;
            RobotEventListener.currentScheme = mNextScheme;
    ...
    static public RobotEventListener eventHandler
    {
        get
        {
            for (int i = 0; i < list.size; ++i)
            {
                ...
                RobotEventListener cam = list.buffer[i];
    ...
    static int CompareFunc(RobotEventListener a, RobotEventListener b)
    {
    ...
    static public RobotEventListener FindCameraForLayer(int layer)
    {
...
#region Event
    ...
    /// 注册动作编辑的事件
    /// </summary>
    public static void RegisterActionEditEvent()
    {
#if Close_Robot
        onSelectDelegate = null;
        onDragDelegate = null;
#else
        onSelectDelegate = OnSelectDlgt;
        onDragDelegate = OnDragDlgt;
#endif
    }
    public static void RegisterSetDJEvent()
    {
        if (null != mCurrentSelectDj)
        {
            if (null != mJms)
            {
                mJms.DestroyArrow();
            }
            mCurrentSelectDj = null;
        }
        onSelectDelegate = OnSetIDSelectDlgt;
        onDragDelegate = null;
    }
    static void OnPress(GameObject obj, bool press)
    {

    }
    static void OnSelectDlgt(GameObject obj)
    {
        if (mCanChangeIDFlag)
        {
            if (obj.name.StartsWith(PublicFunction.Duoji_Start))
            {
                if (null != mCurrentSelectDj && mCurrentSelectDj != obj)
                {
                    //MoveSecond.ChangeDiffuse(mCurrentSelectDj, Normal_Color);
                    if (null != mJms)
                    {
                        mJms.DestroyArrow();
                    }
                ...
                if (null != mJms)
                {
                    if (mCurrentSelectDj != obj)
                    {
                        mJms.CreateArrow(obj);
                    }                    
                }
                ...
                if (mCurrentSelectDj != obj)
                {
                    try
                    {
                        ...
                        if (null != robot)
                        {
                            int id = RobotMgr.Instance.FinddjIDBydjNam(robot.Name, obj.name);//PublicFunction.GetDuoJiId(obj.name);
                            EventMgr.Inst.Fire(EventID.Select_Duoji_For_Robot, new EventArg(id));
                        }
                    }
                ...
                mCurrentSelectDj = obj;
            }
        }
        
    }

    static void OnSetIDSelectDlgt(GameObject obj)
    {
        if (obj.name.StartsWith(PublicFunction.Duoji_Start))
        {
            try
            {
                if (null != mCurrentSelectDj)
                {
                    Robot robottemp = RobotManager.GetInst().GetCurrentRobot();
                    string goType = RobotMgr.Instance.rbt[robottemp.Name].gos[mCurrentSelectDj.name].goType;
                    Dictionary<string, Color> colorTemp = RobotMgr.Instance.FindAllGOColor(robottemp.Name);
                    Normal_Color = colorTemp[mCurrentSelectDj.name];
                ...
                if (null != mJms)
                {
                    MoveSecond.ChangeDiffuse(mCurrentSelectDj, Select_Color);
                    mJms.CreateArrow(obj);
                }
                Robot robot = RobotManager.GetInst().GetCurrentRobot();
                if (null != robot)
    ...
    static void OnClick(GameObject obj)
    {
        if (null != onSelectDelegate)
        {
            onSelectDelegate(obj);
        }
        
    }
    static void OnDragDlgt(GameObject obj, Vector2 delta)
    {
        return;
        if (!mCanChangeRotaFlag)
        {
            return;
        }
        float dis = 0;
        ...
        try
        {
            ...
            {
                int id = RobotMgr.Instance.FinddjIDBydjNam(robot.Name, obj.name);//PublicFunction.GetDuoJiId(obj.name);
                if (null != mCurrentSelectDj && null != mJms)
                {
                    string djName = mCurrentSelectDj.name;
                    Debug.Log("test03");
                    int rota = mJms.Rotate(djName, mJms.FindRotateGO(djName), (int)(dis * Time.deltaTime * 1.3f));
                   
                    EventMgr.Inst.Fire(EventID.Adjust_Angle_For_Robot, new EventArg(id, rota));
                }
            }
    ...
    static void OnDrag(GameObject obj, Vector2 delta)
    {
        if (null != mCurrentSelectDj && null != mJms)
        {
            if (null != onDragDelegate)
            {
                onDragDelegate(mCurrentSelectDj, delta);
            }
            /*float dis = 0;
            if (Math.Abs(delta.x) > Math.Abs(delta.y))
            {
            ...
            int rota = mJms.Rotate(obj, dis * Time.deltaTime);
            try
            {
                int id = PublicFunction.GetDuoJiId(obj.name);
                EventMgr.Inst.Fire(EventID.Adjust_Angle_For_Robot, new EventArg(id, rota));
            }
            catch (System.Exception ex)
            {
                Debuger.Log("选中舵机出错" + ex.ToString());
            }*/
        }
    }
#endregion
 ...
 || Application.platform == RuntimePlatform.BlackBerryPlayer
        ...
        EventMgr.Inst.Regist(EventID.Trigger_Robot_Box, TriggerRobotBox);
#if !Close_Robot
        EventMgr.Inst.Regist(EventID.Select_Duoji_For_UI, SelectDuoJiForUI);
        //EventMgr.Inst.Regist(EventID.Adjust_Angle_For_UI, AdjustAngleForUI);
        EventMgr.Inst.Regist(EventID.OpenOrClose_Duoji_ID_Change, ChangeDuoJiIDBox);
        EventMgr.Inst.Regist(EventID.OpenOrClose_Duoji_Rota_Change, ChangeDuoJiRotaBox);
        EventMgr.Inst.Regist(EventID.Robot_Position_Revert, RevertModel);
#endif
    }

    void OnDestroy()
    {
        EventMgr.Inst.UnRegist(EventID.Trigger_Robot_Box, TriggerRobotBox);
#if !Close_Robot
        EventMgr.Inst.UnRegist(EventID.Select_Duoji_For_UI, SelectDuoJiForUI);
        //EventMgr.Inst.UnRegist(EventID.Adjust_Angle_For_UI, AdjustAngleForUI);
        EventMgr.Inst.UnRegist(EventID.OpenOrClose_Duoji_ID_Change, ChangeDuoJiIDBox);
        EventMgr.Inst.UnRegist(EventID.OpenOrClose_Duoji_Rota_Change, ChangeDuoJiRotaBox);
        EventMgr.Inst.UnRegist(EventID.Robot_Position_Revert, RevertModel);
#endif
    }

    void RevertModel(EventArg arg) 
    {
        CleanSelect();
    }
    void CleanSelect()
    ...
    void SelectDuoJiForUI(EventArg arg)
    {
        if (null != mJms)
        {
            try
            {
                ...
                if (null != robot)
                {
                    name = RobotMgr.Instance.FindDJBydjID(robot.Name, id);//PublicFunction.GetDuoJiName(id);
                }
                GameObject obj = mJms.FindGOByName(name);
                if (null != obj)
                {
                    CleanSelect();
                    //MoveSecond.ChangeDiffuse(obj, Select_Color);
                    mCurrentSelectDj = obj;
                    if (null != mJms)
                    {
                        //mJms.DefaultZone(obj);
                        mJms.CreateArrow(obj);
                    }
                }
                else
    ...
    void TriggerRobotBox(EventArg arg)
    {
        try
        {
            CleanSelect();
    ...
    void ChangeDuoJiIDBox(EventArg args)
    {
        try
        {
            bool flag = (bool)args[0];
            mCanChangeIDFlag = flag;
        }
        catch (System.Exception ex)
        {
            System.Diagnostics.StackTrace st = new System.Diagnostics.StackTrace();
    ...
    void ChangeDuoJiRotaBox(EventArg args)
    {
        try
        {
            bool flag = (bool)args[0];
            mCanChangeRotaFlag = flag;
        }
        catch (System.Exception ex)
        {
            System.Diagnostics.StackTrace st = new System.Diagnostics.StackTrace();
...
#endif
        if (null != UICamera.hoveredObject) return;
        current = this;
    ...
    public void ProcessTouch(bool pressed, bool unpressed)
    {
        ...
        else if (currentTouch.pressed == null && (currentTouch.delta.sqrMagnitude != 0f || currentTouch.current != currentTouch.last) && null != mCurrentSelectDj)
        {
            // If the drag process hasn't started yet but we've already moved off the object, start it immediately
            if (!currentTouch.dragStarted && currentTouch.pressStarted)
            {
                currentTouch.dragStarted = true;
            }
            ...
            if (currentTouch.dragStarted)
            {
                Notify(mCurrentSelectDj, "OnDrag", currentTouch.delta);
            }
        }
