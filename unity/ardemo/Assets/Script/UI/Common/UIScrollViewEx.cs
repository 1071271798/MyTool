...
public class UIScrollViewEx : MonoBehaviour
{
    ...
    /// 是否开启滑动系数
    /// </summary>
    public bool openDragFactor = false;
    /// <summary>
    /// 滑动速度系数
    /// </summary>
    public float dragFactor = 1.0f;

    public float dragSpeed = 0.067f;
    ...
    protected CustomGrid mCustomGrid = null;
    ...
    public virtual Bounds bounds
    {
        get
        {
            ...
            if (null != mCustomGrid)
            {
                mBounds = mCustomGrid.CalculatedBounds();
            }
    ...
    public void UpdateScrollbarActive()
    {
        if (horizontalScrollBar != null)
        {
            horizontalScrollBar.alpha = ((showScrollBars == ShowCondition.Always) || shouldMoveHorizontally) ? 1f : 0f;
    ...
    void FindCustomGrid()
    {
        if (null == mCustomGrid)
        {
            mCustomGrid = transform.GetComponentInChildren<CustomGrid>();
        }
    }
    ...
    public bool RestrictToEnd(bool instant) { return RestrictToBoundsEnd(instant, true, true); }
    /// <summary>
    /// Restrict the scroll view's contents to be within the scroll view's bounds.
    /// </summary>
    ...
    public bool RestrictToBoundsEnd(bool instant, bool horizontal, bool vertical)
    {
        Bounds b = bounds;
        Vector3 constraint = mPanel.CalculateEndOffset(b.min, b.max);
    ...
    public void MoveTo(Vector3 pos, bool instant, int moveMs = 0)
    {
        if (!instant && dragEffect == DragEffect.MomentumAndSpring)
        {
            // Spring back into place
            ...
            {
                MyTweenPosition.Begin(mPanel.gameObject, pos, moveMs / 1000.0f);
            }
        }
        else
        {
            ...
            // Clear the momentum in the constrained direction
            if (Mathf.Abs(pos.x) > 0.01f) mMomentum.x = 0;
            if (Mathf.Abs(pos.y) > 0.01f) mMomentum.y = 0;
            if (Mathf.Abs(pos.z) > 0.01f) mMomentum.z = 0;
            mScroll = 0f;
        }
    }
    /// <summary>
    ...
    public void MoveToFinished()
    {
        if (null != mPanel)
        {
            MyTweenPosition mtp = mPanel.gameObject.GetComponent<MyTweenPosition>();
            if (null != mtp)
            {
                mtp.MoveToFinished();
            }
    ...
    /*public void MoveWithinBounds(Vector3 offset, bool instant)
    {
        Bounds b = bounds;
        Vector3 constraint = Vector3.zero;
        if (offset.x < 0 || offset.y < 0)
        {
            constraint = mPanel.CalculateEndOffset(b.min, b.max);
            
        }
        else
        {
            constraint = mPanel.CalculateFrontOffset(b.min, b.max);

        }
        if (constraint.sqrMagnitude > offset.sqrMagnitude)
        {
            constraint = offset;
        }
    ...
    public virtual void SetDragAmount(float x, float y, bool updateScrollbars)
    {
        ...
        if (null != mCustomGrid)
        {
            b = mCustomGrid.CalculatedBounds();
        }
        else
        {
            b = bounds;
    ...
    public virtual void Press(bool pressed)
    {
        ...
        if (enabled && NGUITools.GetActive(gameObject))
        {
            ...
            else
            {
                ...
                if (!mShouldMove && onDragFinished != null)
                {
                    //onDragFinished();
                    //CamRotateAroundCircle._instance.canChangeView = true;
                }
