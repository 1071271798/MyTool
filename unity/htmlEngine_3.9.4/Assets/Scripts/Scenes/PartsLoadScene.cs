...
public class PartsLoadScene : BaseScene
{
    UIScrollView mScrollView;
    Transform mGrid;
    GameObject mItem;

    string mLoadPath = Application.streamingAssetsPath + "/Parts/editor";

    List<string> mFilesPathList;

    Dictionary<string, ItemData> mDataDict;

    public PartsLoadScene()
    {
        mResPath = "partsLoadScene";
    }
    ...
    protected override void OnButtonClick(GameObject obj)
    {
        ...
        {
            SingletonObject<TimeStatisticsTool>.GetInst().EventStart("Load_Res_Parts");
            mCompleteCount = 0;
            foreach (var item in mDataDict)
            {
                SingletonBehaviour<ResourcesLoad>.GetInst().Load(item.Key, LoadResType.Load_Prefab_Asset_Bundle, delegate (object[] arg)
                {
                    if (null != arg && arg.Length > 0)
                    {
                        foreach (var assetItem in arg)
                        {
                            GameObject assetObj = assetItem as GameObject;
                            assetObj.transform.parent = item.Value.objTrans;
                        }
                    }
                    CompleteOneAsset();
                });
        ...
        else if (obj.name.Equals("Btn_IO_Single"))
        {
            ...
            foreach (var item in mDataDict)
            {
                SingletonBehaviour<ResourcesLoad>.GetInst().LoadSingleThread(item.Key, LoadResType.Load_Prefab_Asset_Bundle, delegate (object[] arg)
                {
                    if (null != arg && arg.Length > 0)
                    {
                        foreach (var assetItem in arg)
        ...
        else if (obj.name.Equals("Btn_WWW"))
        {
            ...
            foreach (var item in mDataDict)
            {
                SingletonBehaviour<ResourcesLoad>.GetInst().WWWLoad(item.Key, LoadResType.Load_Prefab_Asset_Bundle, delegate (object[] arg)
                {
                    if (null != arg && arg.Length > 0)
                    {
                        foreach (var assetItem in arg)
        ...
        else if (obj.name.Equals("Btn_Unload"))
        {
            ...
            SingletonBehaviour<ResourcesLoad>.GetInst().UnLoad(LoadResType.Load_Prefab_Asset_Bundle);
        }
    }
    ...
    void CompleteOneAsset()
    {
        ...
        {
            SingletonObject<TimeStatisticsTool>.GetInst().EventFinished("Load_Res_Parts", delegate (long time)
            {
                Debug.Log(time);
            });
        }
    ...
    private class ItemData
    {
        ...
        public void Destroy()
        {
            if (null != objTrans && objTrans.childCount > 0)
            {
                objTrans.DestroyChildren();
            }
        }
    }
}