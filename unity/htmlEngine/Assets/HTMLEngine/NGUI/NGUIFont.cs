...
namespace HTMLEngine.NGUI {
  ...
  public class NGUIFont : HtFont {
      ...
      : base(face, size, bold, italic)
    {
      ...
      whiteSize = (int)(uiFont.CalculatePrintedSize(" .", true, UIFont.SymbolStyle.None).x * size);
      this.whiteSize -= (int)(uiFont.CalculatePrintedSize(".", true, UIFont.SymbolStyle.None).x * size);
    }

    /// <summary>
    /// Space between text lines in pixels
    /// </summary>
    public override int LineSpacing { get { return (int)uiFont.size + uiFont.verticalSpacing; } }
    ...
    public override HtSize Measure(string text) {
      var r = uiFont.CalculatePrintedSize(text, false, UIFont.SymbolStyle.None) * uiFont.size;
