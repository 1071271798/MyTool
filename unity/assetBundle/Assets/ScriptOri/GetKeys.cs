...
public class GetKeys : EditorWindow 
{
    ...
    [MenuItem("Tools/GetKeys")]
    static void Open()
    {
        var win = GetWindow<GetKeys>();
        win.title = "自动补帧工具";
        win.Show();
    }
        ...
        if (motion != null)
        {
            ...
            if (GUILayout.Button("获取关键帧"))
            {
                ...
                foreach (var editorCurveBinding in binding)
                {
                    ...
                    {
                        Debug.Log("time:" + keyframe.time + ";key:" + keyframe.time*60);
                    }
